...
def test_migration_rerun_is_idempotent(tmp_path):
    db = str(tmp_path / "universe.db")
    assert migrate(db) == 4
    assert migrate(db) == 4                           # rerun: no-op, no error
    assert current_version(db) == 4


def test_migration_rolls_back_atomically_on_failure(tmp_path, monkeypatch):
    db = str(tmp_path / "universe.db")
    assert migrate(db) == 4                            # reach current head first
    # craft a failing additive v5 to prove a later migration rolls back cleanly.
    bad = list(MIGRATIONS) + [(5, [
    ...
    with pytest.raises(sqlite3.OperationalError):
        migrate(db)
    assert current_version(db) == 4                   # not advanced
...
def test_v3_backfill_rerun_is_idempotent(tmp_path, monkeypatch):
    ...
    assert migrate(db) == 4                              # rerun: no-op
