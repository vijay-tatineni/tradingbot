...
| ID | Class | One-line |
|----|-------|----------|
| P3-1 | advisory (doc only) | three docs outside declared naming scope |
| P3-2 | mandatory | `cooldown_until` stores a session count, not a date |
| P3-3 | **mandatory** | state + history writes not atomic together |
| P3-4 | mandatory | candidate-source table not consumed in selection |
| P3-5 | mandatory | portfolio heat ignores inherited/open-book exposure |
| P3-6 | mandatory | canonical-ID collision risk |
| P3-7 | **mandatory** | IBKR mapping check ignores verification status |
| P3-8 | **mandatory** | provider removal can preserve stale open-position state |
| P3-9 | **mandatory** | cooldown depends on observing `POSITION_EXITED_TODAY` |
| BLOCKER-S | mandatory | hypothetical sizing not FX-normalized (from P2-2) |
...
1. All **mandatory** items above (P3-2..P3-9, BLOCKER-S) are implemented and tested.
2. The position-status seam, candidate-source wiring, canonical identity, and
   state/history atomicity are independently reviewed.
...
This commit changes none of that posture: the foundation remains default-off and un-wired.
